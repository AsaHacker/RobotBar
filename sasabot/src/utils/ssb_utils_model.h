...
class NeckModel : public Model<ssb_common_vec::VecNeck,
                               ssb_common_vec::VecNeck,
                               ssb_actuators_dynamixel::NeckDynamixel> {
 public:
  explicit NeckModel(ros::NodeHandle &nh) : Model(nh) {
    setParams(ssb_common_enum::DEBUG);
  };
  ~NeckModel() {};
  void setParams(ssb_common_enum::Config settings);
  void Input2Output();
  ssb_common_vec::VecNeck getHomeAsInput();
 private:
  ssb_common_vec::VecNeck k_input2output_;
};
